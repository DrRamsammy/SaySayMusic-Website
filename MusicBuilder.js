...
</script><script>(function(){var e='https://teachers.saysayeducation.com/api/business/event',v='';try{v=localStorage.getItem('ss_visitor_id')||crypto.randomUUID();localStorage.setItem('ss_visitor_id',v)}catch(x){v=String(Date.now())}function m(t,d){var b=JSON.stringify(Object.assign({event_type:t,product:'music',visitor_id:v,path:location.pathname,source:'music-builder'},d||{}));try{navigator.sendBeacon(e,new Blob([b],{type:'text/plain'}))}catch(x){}}m('page_view');document.addEventListener('click',function(x){var b=x.target.closest&&x.target.closest('button');if(!b)return;var t=String(b.textContent||'').toLowerCase();if(t.includes('generate')||t.includes('create'))m('ai_generation')})})();</script></body></html>`;
